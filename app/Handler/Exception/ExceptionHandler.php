...
//			}
			...
			if (substr($route, 0, 12) == '/admin-login' || $route == '/') {
				return $this->getResponse()->html(Container::singleton(View::class)->render('@public/index'));
			}
			...
			if (substr($route, 0, 8) === '/chapter') {
				...
				if (!$session->get('user')) {
					...
					if ($document && $document->is_public != Document::PUBLIC_DOCUMENT) {
						$this->getResponse()->redirect($this->getLoginUrl(\W7\Core\Facades\Config::get('common.api_host') . ltrim($route, '/')));
					}
				}
			}
			return $this->getResponse()->html(Container::singleton(View::class)->render('@public/index'));
		...
		if (empty($defaultLoginChannel['default_login_channel'])) {
			...
			if (!$setting) {
				...
				return $socialite->config(new Config([
					...
					'redirect_url' => \W7\Core\Facades\Config::get('common.api_host') . 'login?app_id=' . $defaultLoginChannel['default_login_channel'] . '&redirect_url=' . $redirectUrl
